...
RUN apt-get update && apt-get install -y --no-install-recommends sudo openjdk-11-jdk
ENV GODOT_VERSION "3.4.4"
ENV ANDROID_COMPILE_SDK 30
ENV ANDROID_BUILD_TOOLS 30.0.3
...
RUN  /opt/android-sdk/cmdline-tools/bin/sdkmanager --sdk_root=/opt/android-sdk/cmdline-tools "platform-tools" "platforms;android-$ANDROID_COMPILE_SDK" "build-tools;${ANDROID_BUILD_TOOLS}"
